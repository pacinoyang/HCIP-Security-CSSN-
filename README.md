...
云安全向着智能化、自动化、端云协同发展。三层防护：DC边界安全、租户边界安全、租户内安全。

零日漏洞是指还没有补丁的安全漏洞，是APT攻击的主要部分。
沙箱是防范APT攻击的最佳手段之一。
沙箱检测步骤：信誉体系>启发式检测>虚拟执行>威胁分析。
轻量级沙箱是针对某种特定文件进行检测的环境，主要为启发式沙箱（PE/WEB/PDF）。
重量级沙箱是基于虚拟机构造的真实操作系统。

大数据处理+机器学习+安全分析=大数据智能安全分析CIS

数据采集流程：流量采集、协议解析、文件还原和流量元数据上报。

数据处理：数据预处理原理（格式化处理）、分布式存储原理（异构数据分类存储）、分布式索引原理（建立索引）、事件关联分析（事件之间的关联和时序）、流量基线异常检测（内部主机之间、区域之间、内外网之间的访问规则）、WEB异常检测、邮件异常检测、C&C异常、隐蔽通道异常检测、威胁判定。
